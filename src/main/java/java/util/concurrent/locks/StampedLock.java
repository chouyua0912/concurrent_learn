...
public class StampedLock implements java.io.Serializable {
     ...
     *
     * @return a stamp that can be used to unlock or convert mode,          返回信戳 stamp
     * or zero if the lock is not available                                 获取失败返回 0
     */
    public long tryReadLock() {
        for (;;) {
            ...
            if ((m = (s = state) & ABITS) == WBIT)
                return 0L;
            else if (m < RFULL) {   // RFULL = 126                               还有读取的信号剩余
                if (U.compareAndSwapLong(this, STATE, s, next = s + RUNIT))     // 读取每次家1
                    return next;
            }
    ...
    public long tryOptimisticRead() {
        long s;                                                 // state & WBIT ！= 0 说明state的WBIT位置也是1，已经被写线程持有了，直接
        return (((s = state) & WBIT) == 0L) ? (s & SBITS) : 0L; // 读锁不被改变就可以不用释放
    }

    /**
     * Returns true if the lock has not been exclusively acquired               校验发放的信笺有没有失效
     * since issuance of the given stamp. Always returns false if the           issuance：发行，发布
